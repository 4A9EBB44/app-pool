...
<center><span>**Search Apple Name <i style="color: red">Correctly</i> to get its Apple ID and Password!!!**</span><br>
    Nothing displayed? Typo? If not, click the Wishlist on the top Right to submit it to us!</center>

<!-------------------------------------------- Search PHP ----------------------------------------->
<?php
    $mysqli = mysqli_connect('localhost','root','','app-pool');

    $searchApp = mysqli_query($mysqli,"SELECT * FROM home WHERE UPPER(icon) LIKE UPPER('$_POST[search]')");

    while($data = $searchApp->fetch_assoc()){
    echo "
        <div class='app-grid'>
            <table>
                <tr>
